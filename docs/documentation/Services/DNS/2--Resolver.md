...
!!! Warning  "Attention"
    Avec Debian 13, si le service dhcpd est présent, il n'est pas possible d'éditer directement le fichier /etc/resolv.conf qui est en lecture seule. Ainsi, il faudra renseigner les paramètres fournis ci-dessus dans le fichier **/etc/resolv.conf.head**. 
